...
which is the approach taken by LINCAL, we can treat the complex unknown parameters and their complex conjugates as separate unknowns instead. In the latter approach 
we make use of the gradient-based complex Levenberg-Marquardt algorithm and not complex Gauss-Newton for the same reason stated earlier. 
...
direction implicit method outperforms the preconditioned conjugate gradient method, however the preconditioned conjugate gradient method is more robust as it is easier to adapt to other calibration use cases. 
The reason we use the complex optimization framework, instead of the phase-amplitude approach, in this paper is: in this framework
it becomes demonstrate that the alternating direction implicit method is in fact a special case of the complex Levenberg-Marquardt algorithm.
\end{abstract}
