...
[Redundant calibration has been in use for a long time. In the early 1980's it was already used to produce high-dynamic range images of 3C84.] RW The first 
implementations of redundant calibration made use of the logarithm function, because when the logarithm is applied to the scalar measurement equation redundant 
...
LOGCAL is reviewed in Appendix ??. The next improvement was to perform a firt-order taylornomial expansion (which is equivalent to the GN algorithm Kurien 2016) of the measurement equation an then to solve for the
...
and in doing so speedup the algorithm itself was proposed in . A proper comparison of between these two approaches have not been done before.
