...
towards each model parameter, i.e. these methods make use of a Jacobian matrix $\bJ$. In this paper we will be focusing on the following gradient-based minimization algorithms: GN and LM \citep{Levenberg1944,Marquardt1963}.
...
\citet{Smirnov2015} recently proposed to use the Wirtinger derivatives to perform normal skymodel-based calibration. Eq.~\eqref{eq:least_squares} can be recast to
\begin{equation}
...
\end{align}
We refer to $\bJ$ as the Jacobian matrix. Note that the Wirtinger derivatives (see Eq.~\eqref{eq:wir}) are employed in Eq.~\eqref{eq:Jacobian}.
The complex LM update rule is very similar, the major difference being a damping factor $\lambda$ is introduced:
\begin{equation}
\label{eq:LM_update}
...
Eq.~\eqref{eq:GN_update} or Eq.~\eqref{eq:LM_update} is used to iteratively update the 
parameter vector $\breve{\bg}$:
\begin{equation}
\label{eq:update}
\breve{\bg}_{k+1} = \breve{\bg}_{k} + \Delta \breve{\bg}_{k}. 
\end{equation}
The parameter vector is updated until convergence is reached. To improve readability we do not explicitly denote the iteration subscript $k$ in 
