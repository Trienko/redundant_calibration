...
\end{align}
In Eq.~\eqref{eq:vec_definitions}, the size of the three vectors are equal to $B$, which denotes the number of baselines in the array. Moreover,
\begin{equation}
\alpha_{pq} =
\begin{cases}
...
For each composite index we then compute the right hand side of the appropriate equation in Eq.~\ref{eq:vec_definitions} which we then need to assign to the 
correct position in the vector. The correct position is calculated with $\alpha_{pq}$. Using the aforementioned order, however, allows you to fill the array in numerical order, i.e. $12,13,\cdots,1N,23,\cdots \xrightarrow{\alpha_{pq}} 1,2,3,\cdots,B$.  
...
\begin{tikzpicture}[mymatrixenv]
    \matrix[mymatrix] (m)  {
        \frac{\partial v_{pq}}{\partial \eta_j} & \frac{\partial v_{pq}}{\partial \varphi_k}\\
    };
    \mymatrixbraceleft{1}{1}{A}
    \mymatrixbracetop{2}{2}{$j=1,\ldots,N$}
\end{tikzpicture}
\end{equation}
