...
of antennas in the array for different geometries. We use the same color scheme in the right and left panels. We compute $c$ using Eq.~\eqref{eq:c}. We can again compute 
an analytic expression (dashed red line) for the east-west grid using Eq.~\eqref{eq:c} and Eq.~\eqref{eq:gamma}.  
\label{fig:sup_anti}} 
\end{figure*}
