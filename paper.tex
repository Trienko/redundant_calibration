...
and $n_{pq}$ is the thermal noise component. Eq.~\eqref{eq:vis_definition} is known as the scalar measurement equation. Conjugation is denoted by $\conj{(*)}$. The true value of $g_p$, $g_q$ and $y_{pq}$ are unknown. We try to estimate the value of
...
is a redundant array we therefore only require single index values for each redundant baseline group, instead of having a unique single index label for 
each and every baseline. Let $\phi_{pq}$ be the function that maps composite antenna pairs to the appropriate single redundant baseline group index. Note that $\phi_{pq}$ is not symmetric as 
...
where
\begin{align}
 \by &= [y_1,\cdots,y_L]^T, & \bg &=[g_1,\cdots,g_N]^T,\nonumber\\
...
We plot the number of unique baseline groups $L$ and the number of parameters we need to estimate $P$ for two different geometric layouts in Fig~\ref{fig:pl}.
...
a function of the number of antennas $N$ in the array. In the right panel we plot the total number of parameters we need to estimate when employing redundant calibration for a hexagonal (red curve) and regular east-west layout (blue curve) as 
...
The following SNR (signal-to-noise ratio) definition is used in this paper  
\begin{equation}
\label{}
...
HERE I CAN WRITE SOMETHING ABOUT THE LIMITATIONS OF THE PAPER.
...
In Fig.~\ref{fig:gamma} we plot the sparsity ratio for three different geometric layouts as a function of the number antennas in the array. Although, we can not explicitly calculate 
the limit of the hexagonal and square layouts, Fig.~\ref{fig:gamma} seems to indicate that these limits do exist.  

We can now use the sparsity factor to compute the order of the computational complexity of inverting $\bmH$ with
\begin{equation}
P^{c} = (1 - \gamma)P^2.
\end{equation}
...
\end{align}
In Fig.~\ref{fig:c} we plot the value of $c$ for three different geometric layouts as a function of the number antennas in the array.
It is clear from Eq.~\eqref{eq:c} and Fig.~\ref{fig:c} that the computational complexity, under the assumption that $\lim_{N\rightarrow \infty} \gamma$ exists, is asymptotically bounded by $O(P^2)$. However, the computational complexity converges 
very slowly to its asymptotic value, and in general is equal to $O(P^{c})$, where $c \sim 1.7$ if a Hexagonal geometric layout containing less than 200 antennas is employed.
...
of this expression, Eq.~\eqref{eq:gamma}, as $N\rightarrow \infty$ produces the black dotted line. In the right panel we plot the order of the computational cost $c$ for inverting $\bmH$ as a function of the number 
