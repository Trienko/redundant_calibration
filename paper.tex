...
that LINCAL is effectively the non-linear least-squares Gauss-Newton algorithm. An easy way to reduce the number of layers that are currently used would therefore be to employ the Levenberg-Marquardt algorithm instead. We further show that instead of splitting our calibration problem into its phase and amplitude components,
which is the avenue taken by LINCAL, we can treat the complex unknown parameters and their complex conjugates as separate unknowns instead.    
