...
The PCG method differs from the ADI method presented in Section~\ref{sec:adi} in that in the case of PCG we compute the exact inverse of the modified Hessian instead 
of using some sort of approximation as is the case for ADI. As we mentioned in Section~\ref{sec:pcg} there
are two factors which influence the computational complexity of the PCG method, namely the spectral condition number and the sparsity of the matrix we wish to invert. 
...
We respectively discuss the spectral condition number and the sparsity of the modified Hessian, which is a non-singular matrix in Section~\ref{sec:scn} and Section~\ref{sec:sparsity}.
It is interesting to note, that the Hessian itself is singular which means that if we are to use Eq.~\eqref{eq:GN_update} instead we would need to be able invert a singular matrix. In Section~\ref{sec:normal},
we showed that the PCG method can in fact be used to invert a singular Hessian matrix.
