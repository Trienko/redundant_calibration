...
\end{align}
Moreover, $\bmath{\eta},~\bvarphi,~\widetilde{\bmath{\eta}}$ and $\widetilde{\bvarphi}$ are real valued vectors.
If we Taylor expand Eq.~\eqref{eq:lincal_vec} we obtain
\begin{align}
\bd &= \bv + \Delta \bvarrho \bmJ + \bn\\
\br &= \Delta \bvarrho \bmJ + \bn,
\end{align}
where
\begin{equation}
...
\begin{align}
 \left [ \bd \right]_{\alpha_{pq}} &= d_{pq}, & \left [ \bv \right ]_{\alpha_{pq}} &= g_p y_{\phi_{pq}} \conj{g_q}\nonumber\\
 \left [ \bn \right ]_{\alpha_{pq}} &= n_{pq}. &  &\label{eq:lincal_definitions}
\end{align}
