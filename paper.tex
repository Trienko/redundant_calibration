...
In general there are two ways of indexing the visibilities associated with different baselines. We can use the composite index $pq$, which is the strategy that is followed in Eq.~\eqref{eq:vis_definition},
or we can use a single unique baseline index which we assign in numerical order to each composite index; assuming the composite indexes are placed in standard baseline ordering.
The composite indexes $pq$ are ordered using standard baseline ordering 
