...
the number of major iterations that CG requires to converge\footnote{If the $\bA$-norm is used to calculate the error.} is proportional to $\sqrt{\kappa}$, where $\kappa$ is the specctral condition number of the matrix $\bA$.
...
\begin{enumerate}
\item $\bB^H\bB$ is an Hermitian positive semi-definite square matrix by construction.
...
Recall that we mentioned that in general we can only solve Eq.~\eqref{eq:linear_system} with CG if $\bA$ is an Hermitian positive-definite square matrix. Recently,
\citet{Lu2015} showed that the CG method has an even broader applicability, it can be used to solve Eq.~\eqref{eq:linear_system} even if $\bA$ is an Hermitian positive semi-definite
square Hermitian matrix as long as $\bb$ is in the column range of $\bA$. Combining the result from Lu with the aformentioned two observations we made in regards to Eq.~\eqref{}  

It now trivially follows \citet{Lu2015} from follows that we can apply the CG method to solve any normal equation. We have already  


%It is a well known fact that the normal equation has at least one solution. If $\bB^H\bB$ is invertible then the solution that minimizes Eq.~\eqref{eq:norm_ls} is unique. In contrast, if $\bB^H\bB$ is singular there are an infinite amount of solutions
%that minimizes Eq.~\eqref{eq:norm_ls}. 
%Interestingly enough, we can use the CG method to solve Eq.~\eqref{eq:normal_equation} in either one of the previously mentioned scenarios.


Moreover, we can use the CG method to solve Eq.~\eqref{eq:normal_equation}, since $\bA^H\bA$ is Hermitian by construction 
