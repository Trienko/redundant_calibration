...
The conjugate gradient method is an iterative method which is used to solve large linear systems, i.e.
\begin{equation}
\label{eq:linear_system}
\bb = \bA\bx,
\end{equation}
where $\bb$ is a known complex column vector, $\bx$ is the unknown complex column vector with the same length as $\bb$, and $\bA$ is a square complex matrix with appropriate dimensions.  
The conjugate gradient method can in general only be applied to the above linear system if $\bA$ is a square positive-definite or a positive semi-definite Hermitian matrix. If
$\bA$ is a square positive semi-definite Hermitian matrix, then the conjugate gradient method only converges if $\bb$ is in the column range of $\bA$ (cite Lu2016). Moreover, using the 
conjugate gradient method to solve Eq.~\eqref{eq:linear_system} will only be computationally advantageous if $\bA$ is also sparse (contain many zero entries).

The Conjugate Gradient method was independently discovered by Hestenes and Stiefel. They later published a joint paper, n, which is now considered as the seminal
reference on CG. Kaniel was able to determined convergence bounds for CG using Chebyshev polynomial. A much more in depth analysis of CG convergence was conducted 
by van erSluis and van der Vorst. Reid is credited for popularizing CG as an iterative method that can be used to solve sparse linear systems. 

In the early fifties, the method of Conjugate Gradients
...
\begin{algorithm}
\caption{Conjugate Gradient Method}\label{algo:extrap_algo}
\begin{algorithmic}[1]
\State $\br_0 \gets \bA\bx_0 - \bb$
\State $\bp_0 \gets -\br_0$
\State $\bmath{u} \gets \text{linspace}(u_{\text{min}},u_{\text{max}},u_{\text{dim}})$
\State $\bmath{v} \gets \text{linspace}(v_{\text{min}},v_{\text{max}},v_{\text{dim}})$ 
\State $g_{pq} \gets \text{zeros}((u_{\text{dim}},v_{\text{dim}}),\text{dtype}=\text{complex})$ \Comment {define your 2D extrapolated regular $uv$-grid}
\State $i \gets 1$
\For{$i\leq u_{\text{dim}}$}  
\State $j \gets 1$
\For{$j\leq v_{\text{dim}}$}  
\State $\bb \gets (\bmath{u}_i,\bmath{v}_j)^T$
%\State $\Rcal \gets \left\{ \sum_{k\in\mathcal{K}}A_k e^{-2\pi i [\widehat{X}_{rs} \circ \widehat{X}^{-1}_{pq}(\bb)]^T\cdot\bs_k} \right \}_{rs}$
%\State $\Mcal \gets \left\{ \sum_{h\in\mathcal{H}}A_h e^{-2\pi i [\widehat{X}_{rs} \circ \widehat{X}^{-1}_{pq}(\bb)]^T\cdot\bs_h} \right \}_{rs}$
%\Comment {with $\mathcal{K}$ containing the index values of all the sources in the field of view and $\mathcal{L}$ containing the index values of the sources that are included in the
%calibration model}
%\State $\Gcal \gets \text{calibrate}(\Rcal,\Mcal)$
%\Comment {any calibration algorithm can be used here (we assume here that the algorithm outputs $\Gcal$)}
%\State $g_{pq}^{ij} \gets [\Gcal]_{pq}$
\State $j \gets j + 1$
\EndFor
\State $i \gets i + 1$
\EndFor
