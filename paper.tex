...
We can now formulate calibration as an optimization problem:
\begin{equation}
\label{eq:least_squares}
\min_{\bg} \Lambda(\bg) = \min_{\bg} \|\br\|_F^2 = \min_{\bg} \|\bd - \bv(\bg)\|_F^2, 
\end{equation}
where $\bd$ is the observed visibility vector, $\bv$ is the model predicted visibility vector and $\bg =[g_1,\cdots,g_N]^T$. Closely inspecting Eq.~\eqref{eq:least_squares},
we realize that it is in actual fact a least-squares problem, i.e. $\Lambda$ is the objective function, $\br$ is the residual vector, $\bd$ is the observed data vector, $\bv$ is the 
model-predicted data vector and $\bg$ is the model-parameter vector that minimizes the objective function.

The gradient-based minimization algorithms that are generally used to solve least squares problems generally require the model (in Eq.~\eqref{eq:least_squares} the model is denoted by $\bv$) to be differentiable
towards each model parameter. In this paper we will be focusing on the following gradient-based minimization algorithms: GN and LM \citep{Levenberg1944,Marquardt1963}.
...
The only way to circumvent the differentiability conundrum associated with complex least square problems is to recast our complex optimization problem in such a way that $\bv$ becomes analytic in its argument.
The standard way of achieving this is to divide the complex optimization problem into its real and imaginary parts and then to solve for the real and imaginary parts of the parameters separately.
...
\citet{Smirnov2015} recently proposed to use the Wirtinger derivative to perform normal skymodel-based calibration. To achieve this we first recast Eq.~\eqref{eq:least_squares} to
\begin{equation}
...
Least-squares problems are generally solved by using gradient based minimization algorithms. In this paper we will be focusing on the following gradient minimization algorithms: GN and LM \citep{Levenberg1944,Marquardt1963}.


To employ gradient based algorithms the objective function of a least-squares problem has to be differentiable with respect to the model parameters, which is not the case in 
Eq.~\ref{eq:least_squares}. The most general way to deal with this difficulty is to split the least-squares problem into its real and imaginary parts and then to solve 
for the real and imaginary parts of the model-parameter vector separately.
...
%We will mainly accomplish this by using the complex framework proposed by \citet{Smirnov2015}.
...
\citet{Marthi2014}. %and in doing so we have confirmed shown that this algorithm is in actual fact nothing more than redundant \textsc{StEfCal}, i.e. some form of ADI method, justifying the naming convention we have been using for it.
%Re-deriving the algorithm presented in \citet{Marthi2014} in this way serves as a further confirmation that it is indeed nothing more than redundant \textsc{StEfCal} (i.e. some kind of ADI method). 
