...
For example,
\begin{equation}
\{\alpha_{12},\alpha_{13},\alpha_{1N},\alpha_{23},\cdots,\alpha_{N-1N}\} = \{1,2,N-1,N,\cdots,B\},
\end{equation}
where $B$ denotes the number of baselines in the array, i.e. $B = \frac{N^2-N}{2}$. The vectors in Eq.~\eqref{eq:vec_definitions} are column vectors of size $B$ (i.e. $p<q$).
...
\end{equation}
where $\bg =[g_1,\cdots,g_N]^T$.

least squares problems are normally solved by using gradient based methods, like Gauss-Newton and Levenberg-Marquardt .
