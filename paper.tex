...
Table~\ref{tab:fixed_parm}. The sky-model we used to simulate the visibilities were the same for both setups. We generated 100 sources, each source had random 
flux and position. The flux of the sources followed a Pareto distribution (with a shape parameter of 2), while their positions were uniformly 
distributed in a 3 by 3 degree square grid around the field center. We used a hexagonal redundant layout for our antenna positions. We produced 
...
\begin{table}
\centering
\caption{We generated results using two major simulation setups. We either used one frequency channel and multiple time-slots, or one time-slot and multiple frequency 
channels. The most important parameters used in realizing these two major setups are presented here.}
\begin{tabular}{|c c c|} 
\hline
 & Setup 1 & Setup 2\\
\hline
...
performed our simulations over multiple time-slots and one frequency channel or one timeslot and multiple frequencey channels. We use $U$ to represent 
...
\begin{align}
\gamma &= \frac{5N^2-7N+3}{8N^2-8N+2} & \gamma_{\infty} &= \lim_{N\rightarrow \infty}\gamma = \frac{5}{8}. 
\end{align}
...
\begin{align}
c &= \log_{P}(1 - \gamma) + 2 & c_{\infty} &= \lim_{N\rightarrow \infty} c = 2.
\end{align}
