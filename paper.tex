...
from the paper itself. We will elaborate on this later on in Section~\ref{sec:adi}. The second is to use the preconditioned conjugate gradient (PCG) method. PCG allows us to exploit the sparsity inherent in the redundant calibration problem, 
...
The main flow of a single simulation is as follow. We created corrupted visibilities, using the fixed simulation parameters presented in Table~\ref{tab:fixed_parm} and either one of the experimental setups summarized in Table~\ref{tab:ch_parm}. We then used per time-slot and channel redundant calibration, i.e. we use the LM algorithm which in turn makes use of either the ADI or PCG method, to 
...
In Section~\ref{sec:red_c}, we re-derive the fast redundant calibration algorithm presented in \citet{Marthi2014} using the complex framework proposed by \citet{Smirnov2015}. In \citet{Marthi2014} it is derived by taking the derivative of the 
...
To elaborate a little, the conclusion that the algorithm presented in \citet{Marthi2014} is nothing more than a redundant extension of the skymodel-based ADI method (i.e. redundant \textsc{StEfCal}) is in actual fact not obvious either (something we only briefly mentioned in the introduction and then for the sake of simplicity and convenience chose to ignore until now) if the aforementioned 
partial derivative strategy is used to derive it. In Section~\ref{sec:red_c}, we will, therefore, also explicitly show that 
the redundant calibration algorithm presented in \citet{Marthi2014} is in actual fact nothing more than an ADI method and in doing so we will have justified the naming 
convention we have used throughout the paper.
...
gain estimator associated with \textsc{StEfCal}. We have therefore successfully used the diagonal approximation approach proposed by \citet{Smirnov2015} to re-derive the fast redundant calibration method presented in 
\citet{Marthi2014} and in doing so we have shown that this algorithm is in actual fact nothing more than the ADI method (i.e. redundant \textsc{StEfCal}), which justifies the naming convention we have been using for this method. We could also have shown this same result by using
a more direct approach. We could simply have used the alternating strategy presented in the skymodel-based ADI paper itself \citep{Salvini2014}.
...
ADI method can accurately estimate the antenna gains and the observed visibilities at different SNR values. To quantity the exact error in our estimates we used the percentage error $\beta$ defined 
as 
\begin{equation}
\label{eq:beta}
