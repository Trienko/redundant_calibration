...
and $n_{pq}$ is the thermal noise component. Conjugation is denoted by $\conj{(*)}$. During an actual observation the true value of $g_p$, $g_q$ and $y_{pq}$ are unknown and are the quantities which have 
to be estimated.
...
efficiency factor, $\Delta \nu$ is the observational bandwidth and $\tau$ is the integration time per visibility. The  
