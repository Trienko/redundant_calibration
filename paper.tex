...
The first oreder Taylor expansion of Eq.~\eqref{eq:lincal_vec} around the fiducial point $\bvarrho_0$ is equal to
\begin{equation}
\label{eq:d_lin}
\bd = \bv_0 + \bmJ_0 \Delta \bvarrho  + \bn.
\end{equation}
Calculating the residual and dropping the reference to a specific fiducial point, we obtain 
\begin{equation}
\label{eq:r_lin}
...
The augmented residual is equal to 
\begin{equation}
\breve{\br} = \bJ \Delta \bvarrho + \bn, 
\end{equation}

where $\breve{\br} = [r,\conj{r}]^T$ and 
\begin{equation}
\bJ =
\begin{bmatrix}
 \bmJ\\
