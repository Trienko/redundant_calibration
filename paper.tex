...
by preconditioning we reduce $\kappa$ which improves the convergence rate of CG. The best choice of $\bM$ would be $\bA$, which would reduce $\kappa$ to unity which reduces
Eq.~\eqref{eq:cg_bound} to $\mathbb{O}(m)$. But of course, if we knew the inverse of $\bA$ then obtaining $\bx$ would be trivial and we would not need CG or any other type of 
linear solver for that mattter to compute it in the first place. If the diagonal entries of $\bA$ is much more significant than its off-diagonal entries then the 
