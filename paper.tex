...
reference on CG. Kaniel was able to determine convergence bounds for CG using Chebyshev polynomials. A much more in depth analysis of CG convergence was conducted 
by van erSluis and van der Vorst. [Reid is credited for popularizing CG as an iterative method that can efficiently solve large sparse linear systems] RW. 
...
In general, when $\bA$ is neither square, positive semi-definite or Hermitian we can use the normal equation 
\begin{equation}
...
\|\bb-\bA\bx\|. 
\end{equation}
The CG method is gaurenteed to converge if we use it to solve Eq.~\eqref{eq:normal_equation}. 
