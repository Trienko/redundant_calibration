...
def plot_prec_err_paper():
    ...
    for k in xrange(len(SNR)):
        ...
        ax.plot(N1,precentage_stef_mean1*100,color[k],lw=2,label="SNR="+str(SNR[k])+" dB")
        if SNR[k] <> 1000:
...
def plot_outerloop_pres():
    ...
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
    ax.plot(N1,outerloop_pcg_mean1,"r",lw=2,label="PCG (SNR=1000 dB)")
    ...
    ax.plot(N1,outerloop_stef_mean1,"b",lw=2,label="Red StEfCal (SNR=1000 dB)")
    ...
    ax.plot(N1,outerloop_pcg_mean2,"m",lw=2,label="PCG (SNR=5 dB)")
    ...
    ax.plot(N1,outerloop_stef_mean2,"g",lw=2,label="Red StEfCal (SNR=5 dB)")
...
def plot_k_new():
    ...
    plt.semilogy(N1,diff1,lw=2.0,label='SNR=1000 dB',c='b')
    plt.semilogy(N2,diff2,lw=2.0,label='SNR=5 dB',c='g')
    plt.xlabel(r'$N$ [antennas]')
