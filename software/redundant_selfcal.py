...
def redundant_stefcal(D,phi,tau=1e-6,alpha=0.333,max_itr=1000):
    converged = False
    N = D.shape[0]
    D = D - D*np.eye(N)
    L = np.amax(phi)
...
def redundant_StEFCal(D,phi,tau=1e-6,alpha=0.333,max_itr=1000):
    ...
    for t in xrange(D.shape[2]):
        print "t= ",t
        z_temp[:,t],c_temp[t],G[:,:,t],M[:,:,t] = redundant_stefcal(D=D[::,t],phi=phi,tau=tau,alpha=alpha,max_itr=max_itr)
...
if __name__ == "__main__":
   ...
   z_cal,c_cal,G_cal,M_cal = redundant_StEFCal(D,s.phi)
